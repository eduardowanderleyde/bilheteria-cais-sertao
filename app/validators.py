...
def validate_environment() -> Dict[str, Any]:
    ...
    if not settings.secret_key:
        errors.append("SECRET_KEY is required - must be set in environment variables")
    elif len(settings.secret_key) < 32:
        warnings.append("SECRET_KEY is too short - should be at least 32 characters")
    
    # Check admin credentials
    if not settings.admin_username:
        errors.append("ADMIN_USERNAME is required - must be set in environment variables")
    
    if not settings.admin_password:
        errors.append("ADMIN_PASSWORD is required - must be set in environment variables")
    elif len(settings.admin_password) < 8:
        warnings.append("ADMIN_PASSWORD is too short - should be at least 8 characters")
    ...
    if not settings.debug:
        if not settings.secure_cookies:
            warnings.append("SECURE_COOKIES should be enabled in production (HTTPS)")
        
        # Additional production checks
        if settings.database_url == "sqlite:///./bilheteria.db":
            errors.append("Default database URL detected - configure DATABASE_URL for production")
